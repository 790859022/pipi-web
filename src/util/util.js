...
export default {
    request: (url, params, callback, type, headers = {}) => {
        type = type || 'get';
        if ((type === 'get') || (type === 'delete')) params = {params}
        return axios[type](url, params, headers).then(res => {
            if (!res) {
                console.log(res)
                return;
            }
            ...
            callback && callback(data, messageCode, code);
    ...
    pureRequest: (url, params = {}, success, fail, type, headers = {}) => {
        type = type || 'get';
